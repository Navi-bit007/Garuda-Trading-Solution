...
The `Scanner & signals` and `Intratrading` pages also offer the `High-conviction long` strategy. It evaluates completed 5-minute candles only and requires a close above EMA 200, EMA 9 > EMA 20 > EMA 50, a close above the previous 20-candle high, RVOL >= 1.5, close above VWAP and open, positive 5-minute change, and candle strength >= 0.60. It uses a 100-point score with a minimum of 85, an ATR(14) stop, +1% target 1, and +2% target 2; target 1 moves the protective stop to breakeven. NIFTY above VWAP contributes the optional market-confirmation points.

The `Scanner & signals` page includes a `Historical day scan` expander that evaluates one saved watchlist on one completed date using daily history through that date. It supports the EMA 9/200 swing, EMA 200 close-above, and previous-day high breakout strategies, returns every matching stock without a result cap, reports symbol-level data errors separately, and can export the result table as CSV.

The `Backtesting` page uses Kite Connect historical candles only and requires Kite authentication. Run one selected symbol or the selected watchlists with Daily or 5-minute candles, choose a strategy from Scanner & signals, Intratrading, or Swing auto trading, and configure the date range, sizing, fees, and slippage. It reports aggregate and per-symbol metrics, equity, exit reasons, and downloadable trades. Backtesting is research-only: it never submits orders or modifies live positions. The EMA 9/200 progressive replay enters only after STRONG alignment, while Swing trend breakout uses next-session confirmation. Kite data errors are reported per symbol.

The `Watchlists` page creates user-owned lists from the full NSE/BSE instrument catalog. Search by company or symbol, add instruments to one or more lists, paste a comma/newline-separated stock list for bulk adding, select multiple lists for signals, and manage each list without importing an index. Bulk stock import resolves only equity instruments on the chosen NSE or BSE exchange and reports symbols it cannot find. An optional bulk index import is available as a convenience. Selected instruments are deduplicated by instrument token before the backend processes them. `Scanner & signals` and `Intratrading` consume persisted backend records only; opening either page is not required for generation. The dashboard reports fresh, stalled, and error heartbeats for the signal engine. Notifications remain persisted for Telegram delivery and JSON backup, but there is no notification page or popover.
